...
function createCard(event, isTop) {
  ...
  card.innerHTML = `
    <div class="card-nav-arrow card-nav-prev">❮</div>
    <div class="card-nav-arrow card-nav-next">❯</div>
  ...
  if (isTop) {
    setupSwipe(card, event);
    setupNavArrows(card, event);
  }
...
function setupNavArrows(card, event) {
...
async function addToPlan(eventId) {
  ...
  try {
    ...
    if (data.ok) tg.showAlert("Добавлено в план!");
...
async function markAttended(eventId) {
  ...
  try {
    ...
    if (data && data.ok) {
      tg.showAlert(`Серия! Твоя серия: ${data.streak} дн.`);
      loadPlan();
    }
  } catch (e) { tg.showAlert("Ошибка: " + (e.message || "Неизвестная ошибка")); }
}
    ...
    if (data.ok) {
      ...
      const statsHtml = `
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:20px;">
          ...
          <div class="stat-item">
            <span class="stat-value">${stats.totalSpent || 0} ₽</span>
      ...
      `;
      document.getElementById("booking-stats").innerHTML = statsHtml;
      ...
      if (bookings.length === 0) {
        ...
        container.innerHTML = bookings.map(b => `
          <div class="booking-item">
            <div class="booking-status ${b.status}">${b.status === "confirmed" ? "✅ Активен" : b.status === "used" ? "🎫 Использован" : "❌ Отменён"}</div>
            <div class="booking-title">${escapeHtml(b.title)}</div>
            <div class="booking-ref">🎟️ ${b.booking_reference}</div>
            ${b.external_url ? `<a href="${b.external_url}" target="_blank" class="btn btn-primary" style="margin-top:10px;display:inline-block;">Открыть билет</a>` : ''}
          </div>`).join("");
      }
    }
  } catch (e) { container.innerHTML = '<div class="empty-state">Нет соединения</div>'; }
...
function renderProfile() {
  ...
  if (attendedEl) attendedEl.textContent = state.profile.stats.totalAttended || 0;
  const likesEl = document.getElementById("stat-likes");
  if (likesEl) likesEl.textContent = state.profile.stats.likes || 0;
  
  // Бейджи
  const badgesContainer = document.getElementById("badges-container");
  if (badgesContainer && state.profile.badges) {
    badgesContainer.innerHTML = state.profile.badges.map(b => `
      <div class="badge-item ${b.earned ? 'earned' : ''}">
        <div class="badge-emoji">${b.emoji || '🏆'}</div>
        <div class="badge-name">${b.name}</div>
        ${b.earned ? `<div class="badge-date">${new Date(b.earned_at).toLocaleDateString('ru-RU')}</div>` : ''}
      </div>
    `).join("");
  }
}
